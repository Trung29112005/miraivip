...
var full = [];
    ...
    var array = [number[0],number[1],number[2]];
        ...
        if (array.includes(itemm)) {
            ...
            if (i == 1) {
                var mon = parseInt(args[1]) + 300;  
                    await Currencies.increaseMoney(event.senderID, mon); console.log("s1")
                        return api.sendMessage({body:`[𝑷𝑮🐧] => Kết Quả : ${full.join("|")}\n[✤] => Được ${mon} Đô,Vì Có 1 ${itemm}!`,attachment: listimg},event.threadID, event.messageID);
            }
            else if (i == 2) {
                var mon = parseInt(args[1]) * 2; 
                    await Currencies.increaseMoney(event.senderID, mon); console.log("s2")
                        return api.sendMessage({body:`[𝑷𝑮🐧] => Kết Quả : ${full.join("|")}\n[✤] => Được ${mon} Đô,Vì Có 2 ${itemm}!`,attachment: listimg},event.threadID, event.messageID);
            }
            else if (i == 3) {
                var mon = parseInt(args[1]) * 3; 
                    await Currencies.increaseMoney(event.senderID, mon); console.log('s3')
                        return api.sendMessage({body:`[𝑷𝑮🐧] => K���t Quả : ${full.join("|")}\n[✤] => Được ${mon} Đô,Vì Có 3 ${itemm}!`,attachment: listimg},event.threadID, event.messageID);
            }
            ...
            return api.sendMessage({body:`[𝑷𝑮🐧] => Kết Quả : ${full.join("|")}\n[✤] => Trừ ${args[1]} Đô,Vì Có 0 ${itemm}!`,attachment: listimg},event.threadID, event.messageID);
        }
            } ,event.messageID);
    };