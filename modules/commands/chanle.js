module.exports.config = {
  ...
  usages: " <chẵn/lẻ/C0/C2/C4/C6/C8/L1/L3/L5/L7/L9> <số tiền cược>\Ví dụ: chanle C2 5000\Trong đó:\+ Nếu đặt chẵn/lẻ số tiền chiến thắng sẽ được x3, nếu thua sẽ mất toàn bộ số tiền cược\+ Nếu đặt C0|2|4|6|8/L1|3|5|7|9 số tiền chiến thắng sẽ được x6, nều thua sẽ mất gấp đôi số tiền cược",
  cooldowns: 1
};
...
module.exports. run = async function ({ args, api, event, Currencies }) {
  ...
  if (!betBox || ![...board.C, ...board.L].includes(betBox)) return reply("Vui lòng gửi tin nhắn theo cú pháp chanle <ô đặt> <số tiền đặt cược>\Gõ /help chanle để xem chi tiết hướng dẫn");
  ...
  if (moneyUser < moneyBet) return reply(`Bạn không đủ tiền để đặt cược\» Số tiền hiện tại của bạn: ${moneyUser}\» Số tiền còn thiếu: ${moneyBet-moneyUser} ${moneyUnit}`);
  ...
  if (WIN) {
    ...
    else if (betBox.length == 2)responseMoney = -moneyBet*1;
  }
  
  reply(`» Kết quả là: ${result}\» ${text[0]} ${text[1]}: ${Math.abs(responseMoney)} ${moneyUnit}\» Số tiền hiện tại: ${moneyUser+responseMoney} ${moneyUnit}`);
