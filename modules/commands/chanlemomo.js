...
, attachment : [
    ...
    if(parseInt(coins) < 100) return api.sendMessage('Min 1000 bé ơi!', threadID, messageID);
    ...
    //xử lí canvas
    let bg = (await axios.get(`https://i.postimg.cc/nVSdp92F/Picsart-22-04-11-08-31-30-554.jpg`, {responseType: "arraybuffer" })).data;
    ...
    ctx.textAlign = "start";
    ctx.fillText('-' + coins.replace(/\B(?=(\d{3})+(?!\d))/g, '.') + 'đ', 66, 95);
    ctx.font = "17px SplineSans";
    ctx.fillText(content, 30, 570);
    ctx.font = "15px SplineSans-Medium";
    ctx.textAlign = "right";
    var info = await api.getUserInfo(event.senderID);
    var name = info[event.senderID].name
    ctx.fillText('VÕ THÀNH TRUNG', 432, 412);
    ctx.fillStyle = "#0066FF";
    ctx.font = "16px SplineSans-Medium";
    const data = [" 0832926084 ", " 0832926084 ", " 0832926084 ", " 0832926084 " , " 0832926084 " , " 0832926084 "];
    var sdt = data[Math.floor(Math.random() * data.length)]
    ctx.fillText(`${sdt}`, 433, 447);
    ctx.font = "13px SplineSans";
    ctx.textAlign = "start";
    ctx.fillText(codeGD, 150, 120);
    ctx.fillStyle = "#000000";
    ctx.textAlign = "right";
    ctx.font = "16px SplineSans-Medium";
    ctx.fillText('Miễn phí', 432, 256);
    ctx.fillText('Ví MoMo', 432, 219);
    ctx.fillText(`${time} - ${day}`, 432, 186);
