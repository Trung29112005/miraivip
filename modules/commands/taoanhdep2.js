module.exports.config = {
	name: "taoanhdep2", // Tên lệnh, được sử dụng trong việc gọi lệnh
	version: "1.0.0", // phiên bản của module này
	hasPermssion: 0, // Quyền hạn sử dụng, với 0 là toàn bộ thành viên, 1 là quản trị viên trở lên, 2 là admin/owner
	credits: "Trung05", // Công nhận module sở hữu là ai
	description: "Tạo ảnh đẹp 2", // Thông tin chi tiết về lệnh
...
module.exports.handleReply = async ({ api, event, handleReply }) => {
	...
	switch (handleReply.step) {
		case 1:
			content.name = input;
			sendC("Reply tin nhắn này id nhân vật", 2, content);
			break;
		case 2:
			content.subname = input;
			sendC("Reply tin nhắn này tên phụ", 3, content);
			break;
		case 3:
			content.color = input;
			...
			api.unsendMessage(handleReply.messageID);
			let c = content;
			let res = await axios.get(encodeURI(`https://www.phamvandienofficial.xyz/fbcover/v2?name=${c.name}&id=${c.subname}&subname=${c.color}`), { responseType: "arraybuffer" })
...
module.exports.run = ({ api, event, args }) => {
	...
	return api.sendMessage("Reply tin nhắn này tên chính", event.threadID, (err,info) => {
		global.client.handleReply.push({
			step: 1,
			name: this.config.name,
			messageID: info.messageID,
