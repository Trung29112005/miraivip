...
let text = args.join(" ")
  if (!text) return api.sendMessage('Vui lòng nhập link tikvd', event.threadID, event.messageID);
...
var url = res.data.data.linkDowload;
	 ...
	 var callback = () => api.sendMessage({body:`Video TikTok Đây`,attachment: fs.createReadStream(__dirname + "/cache/tkvd.mp4")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/tkvd.mp4"),event.messageID);
