...
var text1 = ag[0],
    text2 = ag[1];
if(!text1){
  return api.sendMessage(`Sai cách sử dụng\Nhập =>socialdl help để xem hướng dẫn`, event.threadID)
}
let app = {
  "facebook" : 0,
  "instagram": 1,
...
if (!Object.keys(app).includes(text1)) return api.sendMessage("Lựa chọn không hợp lệ\\Vui lòng nhập =>socialdl help để xem hướng dẫn", event.threadID);
if(text1 == "facebook"){
  const res = await axios.get(`https://www.phamvandienofficial.xyz//videodowload?url=${text2}`);
...
if(text1 == "tiktok"){
  const res2 = await axios.get(`https://www.phamvandienofficial.xyz//videodowload?url=${text2}`);
...
if(text1 == "help"){
  return api.sendMessage(`Hướng Dẫn Sử Dụng Social Download\\-Example : =>socialdl <app> - <link>\\- Các app được hỗ trợ : Facebook, Instagram, Pinterest, Tiktok`, event.threadID);
}
if(text1 == "pinterest"){
